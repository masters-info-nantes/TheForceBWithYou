...
THEORY ListVariablesX IS
  ...
  Local_List_Variables(Machine(dossier))==(historique,traitement_etat,traitement_lib,modification_etat,date_creation,proprietaire,nb_dossier,dates,employes,dossiers,etat_traitement,lib_traitement);
  List_Variables(Machine(dossier))==(historique,traitement_etat,traitement_lib,modification_etat,date_creation,proprietaire,nb_dossier,dates,employes,dossiers,etat_traitement,lib_traitement);
  External_List_Variables(Machine(dossier))==(historique,traitement_etat,traitement_lib,modification_etat,date_creation,proprietaire,nb_dossier,dates,employes,dossiers,etat_traitement,lib_traitement)
END
&
...
THEORY ListInvariantX IS
  ...
  List_Invariant(Machine(dossier))==(lib_traitement <: LIBELLE_TRAITEMENT & etat_traitement <: ETAT_TRAITEMENT & dossiers <: NAT & nb_dossier: INT & employes <: EMPLOYE & dates <: NAT & proprietaire: dossiers --> employes & date_creation: dossiers --> dates & modification_etat: dossiers*dates +-> etat_traitement & traitement_lib: dates*employes --> lib_traitement & traitement_etat: dates*employes --> etat_traitement & historique: dossiers --> (dates*employes +-> lib_traitement*etat_traitement) & card(modification_etat)<=max_op & dom(traitement_lib) = dom(traitement_etat))
END
&
...
THEORY ListInitialisationX IS
  Expanded_List_Initialisation(Machine(dossier))==(lib_traitement,etat_traitement,dossiers,employes,dates,nb_dossier,proprietaire,date_creation,modification_etat,traitement_lib,traitement_etat,historique:={},{},{},{},{},0,{},{},{},{},{},{});
  Context_List_Initialisation(Machine(dossier))==(skip);
  List_Initialisation(Machine(dossier))==(lib_traitement:={} || etat_traitement:={} || dossiers:={} || employes:={} || dates:={} || nb_dossier:=0 || proprietaire:={} || date_creation:={} || modification_etat:={} || traitement_lib:={} || traitement_etat:={} || historique:={})
END
&
...
THEORY ListInputX IS
  ...
  List_Input(Machine(dossier),tracerOperation)==(id_d,id_c,date,lib_op,etat_op);
...
THEORY ListHeaderX IS
  ...
  List_Header(Machine(dossier),tracerOperation)==(tracerOperation(id_d,id_c,date,lib_op,etat_op));
...
THEORY ListPreconditionX IS
  ...
  List_Precondition(Machine(dossier),tracerOperation)==(id_d: NAT & id_c: EMPLOYE & date: NAT & lib_op: LIBELLE_TRAITEMENT & etat_op: ETAT_TRAITEMENT & id_c = proprietaire(id_d));
...
THEORY ListSubstitutionX IS
  ...
  Expanded_List_Substitution(Machine(dossier),tracerOperation)==(id_d: NAT & id_c: EMPLOYE & date: NAT & lib_op: LIBELLE_TRAITEMENT & etat_op: ETAT_TRAITEMENT & id_c = proprietaire(id_d) | modification_etat,traitement_lib,traitement_etat,historique:=modification_etat<+{(id_d,date)|->etat_op},traitement_lib<+{(date,id_c)|->lib_op},traitement_etat<+{(date,id_c)|->etat_op},historique<+{id_d|->(historique(id_d)<+{(date,id_c)|->(lib_op|->etat_op)})});
  Expanded_List_Substitution(Machine(dossier),creerDossier)==(id_c: EMPLOYE & date_c: NAT | @nouvd.(nouvd: NAT & nouvd = nb_dossier+1 ==> proprietaire,date_creation:=proprietaire\/{nouvd|->id_c},date_creation\/{nouvd|->date_c}));
  List_Substitution(Machine(dossier),creerDossier)==(ANY nouvd WHERE nouvd: NAT & nouvd = nb_dossier+1 THEN proprietaire:=proprietaire\/{nouvd|->id_c} || date_creation:=date_creation\/{nouvd|->date_c} END);
  List_Substitution(Machine(dossier),tracerOperation)==(modification_etat(id_d,date):=etat_op || traitement_lib(date,id_c):=lib_op || traitement_etat(date,id_c):=etat_op || historique(id_d)(date,id_c):=lib_op|->etat_op);
...
THEORY ListANYVarX IS
  List_ANY_Var(Machine(dossier),creerDossier)==(Var(nouvd) == btype(INTEGER,?,?));
  List_ANY_Var(Machine(dossier),tracerOperation)==(?);
  List_ANY_Var(Machine(dossier),modifierEtatTraitement)==(?)
END
&
THEORY ListOfIdsX IS
  List_Of_Ids(Machine(dossier)) == (max_op,EMPLOYE,LIBELLE_TRAITEMENT,ETAT_TRAITEMENT,creation,modification,suppression,commence,en_cours,termine | ? | historique,traitement_etat,traitement_lib,modification_etat,date_creation,proprietaire,nb_dossier,dates,employes,dossiers,etat_traitement,lib_traitement | ? | creerDossier,tracerOperation,modifierEtatTraitement | ? | ? | ? | dossier);
